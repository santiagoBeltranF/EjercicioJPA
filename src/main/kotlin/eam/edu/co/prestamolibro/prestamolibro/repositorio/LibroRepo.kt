...
class LibroRepo {
    ...
    fun findByEditorial(code: Int): List<Book> {
        val query = em.createQuery("SELECT boo FROM Book boo WHERE boo.Codigo_Editorial.code=:codeEditorial")
        query.setParameter("codeEditorial", code)
        return query.resultList as List<Book>
    }

    fun createLibro(libro: Book) {
